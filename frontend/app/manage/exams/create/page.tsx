...
export default function CreateExamPage() {
    ...
    const [examType, setExamType] = useState("PRACTICE"); // PRACTICE, MODULE, FINAL, TOPIC_TEST
    ...
    const [questions, setQuestions] = useState<Question[]>([
        { id: '1', question: '', options: ['', '', '', ''], correct_index: 0, explanation: '', marks: 1 }
    ]);
    ...
    const fetchTopics = async (courseId: string) => {
        try {
            ...
            const sortedTopics = res.data.sort((a: Topic, b: Topic) => {
                ...
                if (wa !== wb) return wa - wb; // Sort by weight (Unit I=1, 1.1=1.1, Unit II=2)
                return a.title.localeCompare(b.title); // Fallback to string sort
            });
    ...
    // Handle Cloning Pre-fill
    useEffect(() => {
        const fetchCloneSource = async () => {
            if (!token || !cloneFromId) return;

            try {
                setLoading(true);
                const res = await axios.get(`${API_URL}/exams/${cloneFromId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                const source = res.data;

                // Populate state from source
                setSelectedCourseId(source.course_id || "");
                if (source.course_id) fetchTopics(source.course_id);

                setSelectedTopicId(source.topic_id || "");
                setExamCategory(source.type === 'PRACTICE' ? 'PRACTICE' : 'FORMAL');
                setExamType(source.type || 'PRACTICE');
                setExamScope(source.scope || 'TOPIC');
                setDifficulty(source.difficulty || 'medium');
                setDuration((source.duration_minutes || 30).toString());
                setPassingScore((source.passing_score || 70).toString());
                setTitle(`Copy of ${source.title || 'Exam'}`);

                if (source.questions && Array.isArray(source.questions)) {
                    setQuestions(source.questions.map((q: any, i: number) => ({
                ...
                toast.success("Exam details pre-filled for cloning");
            } catch (error) {
                console.error("Failed to fetch clone source", error);
                toast.error("Failed to load original exam for cloning");
            } finally {
                setLoading(false);
            }
        };
    ...
    // Sync examType based on category and scope
    useEffect(() => {
        if (examCategory === 'PRACTICE') {
            setExamType('PRACTICE');
        }
        // For FORMAL, examType is set directly (TOPIC_TEST, MODULE, FINAL)

        // Clear topic when needed (Only if NOT cloning, to avoid flickering)
        if (!cloneFromId && (examType === 'FINAL' || (examType === 'PRACTICE' && examScope === 'COURSE'))) {
            setSelectedTopicId("");
        }
    }, [examCategory, examType, examScope, cloneFromId]);
    ...
    const validateMetadata = () => {
        ...
        // Topic/Module validation based on exam type and scope
        if (examType === 'MODULE' && !selectedTopicId) {
            toast.error("Please select a module for Module Exam");
            return false;
        }

        if (examType === 'TOPIC_TEST' && !selectedTopicId) {
            toast.error("Please select a topic for Topic Test");
            return false;
        }

        if (examType === 'PRACTICE') {
            if (examScope === 'TOPIC' && !selectedTopicId) {
                toast.error("Please select a topic for Topic Practice");
                return false;
            }
            if (examScope === 'MODULE' && !selectedTopicId) {
                toast.error("Please select a module for Module Practice");
                return false;
            }
            // COURSE scope doesn't need topic selection
        }

        // For FINAL exams, no topic is needed.
        if (!duration || parseInt(duration) <= 0) {
            toast.error("Please enter a valid duration");
            return false;
        }
        if (!passingScore || parseInt(passingScore) <= 0 || parseInt(passingScore) > 100) {
            toast.error("Please enter a valid passing score (1-100)");
            return false;
        }
        return true;
    };
    ...
    const handlePublish = async (isDraft: boolean = false) => {
        ...
        if (!isDraft && !validateQuestions()) return; // Only validate questions if not saving as draft

        setSubmitting(true);
        try {
            // 1. Create Exam Draft
            // Handle Topic ID Logic: Convert "all_topics" or empty string to null
            const finalTopicId = (selectedTopicId === 'all_topics' || !selectedTopicId) ? null : selectedTopicId;

            const createRes = await axios.post(`${API_URL}/exams/manual`, {
                topic_id: finalTopicId,
                module_id: null,  // Explicitly set to null
            ...
            const examId = createRes.data.exam_id;
            ...
            toast.success(isDraft ? "Exam draft saved successfully!" : "Exam created and published successfully!");
            ...
            // @ts-ignore
            toast.error(error.response?.data?.detail || error.message || "Failed to publish exam");
            if (error.response?.data?.detail?.errors) {
                error.response.data.detail.errors.forEach((err: string) => toast.error(err));
            }
        } finally {
            setSubmitting(false);
        }
                ...
                <div className="relative z-10">
                    ...
                    <div className="space-y-4">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-400/30 text-indigo-200 text-xs font-semibold backdrop-blur-md">
                            <Brain className="h-3.5 w-3.5" /> Exam Creator
                        </div>
                        <h1 className="text-4xl font-bold tracking-tight">Design New Assessment</h1>
                        <p className="text-indigo-200 lg:max-w-xl text-lg">
                            Create custom exams with our interactive builder. Set rules, add questions, and publish instantly.
                        </p>
                    </div>
                </div>
            </div>
            ...
            {step === 0 && (
                <Card className="border-0 shadow-2xl shadow-slate-300/50 rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 ring-1 ring-slate-900/5">
                    <div className="bg-slate-50/80 p-6 border-b border-slate-200">
                        ...
                        <p className="text-slate-500 ml-4 font-medium">Define the core settings for this assessment</p>
                    </div>
                    ...
                    <CardFooter className="flex justify-end p-8 bg-slate-50 border-t border-slate-200">
                        ...
                        >
                            Next: Add Questions <ChevronRight className="ml-2 h-5 w-5" />
                        </Button>
                    </CardFooter>
                </Card>
            )}
                        ...
                        <CardContent className="p-8">
                            <div className="flex flex-col md:flex-row gap-6 items-center">
                                ...
                                <div className="space-y-2 text-center md:text-left flex-1">
                                    <h3 className="text-xl font-bold text-indigo-900">Review & Publish</h3>
                                    <p className="text-indigo-700/80 max-w-2xl">
                                        Verify your exam structure and questions below. Once published, students will be able to access this exam immediately. Question contents will become locked after the first submission.
                                    </p>
                                </div>
