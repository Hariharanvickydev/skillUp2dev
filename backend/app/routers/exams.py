...
def get_admin_attempts(
    ...
    if practice_mode:
        ...
        for attempt in all_attempts:
            user_id = attempt.id  # This is models.User.id from the query
            ...
            attempt_number = len(student_groups[user_id]["attempts"]) + 1
            
            student_groups[user_id]["attempts"].append({
                "id": attempt.ExamAttempt.id,
                "attempt_number": attempt_number,
                "score": attempt.ExamAttempt.score,
                "passed": attempt.ExamAttempt.passed,
                "submitted_at": attempt.ExamAttempt.submitted_at
            })
        
        # Calculate improvement indicators and add summary stats
        result = []
        for group in student_groups.values():
            ...
            if attempts:
                ...
                if latest_score > first_score + 10:
                    indicator = "improving"
                elif latest_score < first_score - 10:
                    indicator = "declining"
                else:
                    indicator = "stable"
                
                group["improvement_indicator"] = indicator
        ...
        # For assessments: Keep only latest attempt per student (current behavior)
        seen_users = set()
        unique_attempts = []
        
        for attempt in all_attempts:
            user_id = attempt.ExamAttempt.user_id
